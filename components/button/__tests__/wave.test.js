...
describe('click wave effect', () => {
  async function clickButton(wrapper) {
    ...
    wrapper.find('.bmant-btn').element.dispatchEvent(new Event('transitionstart'));
    await sleep(20);
    wrapper.find('.bmant-btn').element.dispatchEvent(new Event('animationend'));
    await sleep(20);
  }
  ...
  it('should have click wave effect for primary button', async () => {
    ...
    expect(wrapper.find('.bmant-btn').attributes('bmant-click-animating-without-extra-node')).toBe(
      'true',
    );
  });
  ...
  it('should handle transitionstart', async () => {
    ...
    await clickButton(wrapper);
    const buttonNode = wrapper.find('.bmant-btn').element;
